...
export default function App() {
  ...
  const [sttHealth, setSttHealth] = useState<any>(null);
  ...
  async function refreshMicDevices() {
    setMicError("");
    try {
      const [devicesRes, selectedRes, healthRes] = await Promise.all([
        fetch(`${apiBase}/audio/input-devices`),
        fetch(`${apiBase}/audio/input-device`),
        fetch(`${apiBase}/audio/stt-health`),
      ]);
      ...
      if (healthRes.ok) setSttHealth(await healthRes.json());
  ...
  async function refreshSttHealth() {
    try {
      const response = await fetch(`${apiBase}/audio/stt-health`);
      if (response.ok) setSttHealth(await response.json());
    } catch {
      // The global connection status already reports backend outages.
    }
  }
  ...
  useEffect(() => {
    refreshSttHealth();
    const timer = window.setInterval(refreshSttHealth, 2000);
    return () => window.clearInterval(timer);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [apiBase]);
...
function GameIntelligencePanel({ intelligence }: { intelligence: any }) {
  const data = intelligence || {};
  const progress = data.progress_state || {};
  const accepted = Array.isArray(data.facts_accepted) ? data.facts_accepted : [];
  const rejected = Array.isArray(data.facts_rejected) ? data.facts_rejected : [];
  const gaps = Array.isArray(data.unresolved_knowledge_gaps) ? data.unresolved_knowledge_gaps : [];
  const provenance = Array.isArray(data.current_comment_provenance) ? data.current_comment_provenance : [];
  const provenanceLabels = provenance.map((source: any) => `${displayValue(source.title, "source")} — ${displayValue(source.location)}`);
  return <section className="glass panel sessionPanel wide"><div className="panelHeader slim"><div><div className="panelTitle">Game Intelligence</div><div className="panelMeta">grounded research and spoiler firewall</div></div></div><div className="sessionCardGrid"><SessionCard title="Game dossier"><InfoRow label="Current game" value={displayValue(data.current_game)} /><InfoRow label="Dossier" value={displayValue(data.dossier_status)} /><InfoRow label="Spoiler mode" value={displayValue(data.spoiler_mode)} /><InfoRow label="Progress" value={displayValue(progress.current_area || progress.current_chapter || progress.playthrough_type)} /></SessionCard><SessionCard title="Research"><InfoRow label="Last trigger" value={displayValue(data.last_research_trigger?.reason)} /><InfoRow label="Active job" value={displayValue(data.active_research_job?.status)} /><InfoRow label="Last query" value={displayValue(data.last_query)} /><InfoRow label="Sources / cache" value={`${Number(data.sources_found || 0)} / ${Number(data.cache_hits || 0)}`} /></SessionCard><SessionCard title="Fact gate"><InfoRow label="Accepted" value={String(accepted.length)} /><InfoRow label="Rejected" value={String(rejected.length)} /><InfoRow label="Last spoiler block" value={displayValue(data.last_spoiler_block?.reason)} /><InfoRow label="Knowledge gaps" value={displayValue(gaps)} /></SessionCard><SessionCard title="Current comment"><InfoRow label="Mode" value={displayValue(data.current_comment_mode)} /><InfoRow label="Lookup used" value={data.lookup_used ? "yes" : "no"} /><InfoRow label="Supporting facts" value={displayValue(data.current_comment_fact_ids)} /><InfoRow label="Fact provenance" value={displayValue(provenanceLabels)} /></SessionCard></div></section>;
}
...
function STTHealthPanel({ health }: { health: any }) {
  const h = health || {};
  const fmt = (value: any) => typeof value === "number" ? value.toFixed(3) + "s" : String(value ?? "-");
  return <section className="glass panel audioPanel wide"><div className="panelHeader slim"><div><div className="panelTitle">STT health</div><div className="panelMeta">effective runtime profile and rolling performance</div></div></div><div className="statusList"><StatusLine label="Profile" value={`${h.effective_model || "-"} / ${h.device || "-"} / ${h.compute_type || "-"}`} tone={h.engine_status === "error" ? "bad" : h.engine_status === "ready" || h.engine_status === "fallback" ? "ok" : "idle"} /><StatusLine label="Model load" value={fmt(h.model_load_seconds)} tone="idle" /><StatusLine label="Active mode" value={h.active_mode || "-"} tone="idle" /><StatusLine label="Input" value={h.input_device?.device_name || "default"} tone="idle" /><StatusLine label="Language" value={h.detected_language || "-"} tone="idle" /><StatusLine label="Last / avg / p95" value={`${fmt(h.last_latency_seconds)} / ${fmt(h.average_latency_seconds)} / ${fmt(h.p95_latency_seconds)}`} tone="idle" /><StatusLine label="Command confidence" value={Number(h.command_confidence || 0).toFixed(3)} tone="idle" /><StatusLine label="Wake / target" value={`${h.last_wake_decision || "-"} / ${h.last_application_target_resolution?.resolved_app || "-"}`} tone="idle" /><StatusLine label="TTS echo" value={h.tts_echo_state?.active ? "active" : h.tts_echo_state?.tail_active ? "tail" : "clear"} tone={h.tts_echo_state?.active ? "warn" : "ok"} /><StatusLine label="Rejects" value={`hallucination ${h.hallucination_reject_count || 0}, prompt ${h.prompt_echo_reject_count || 0}`} tone="idle" /></div>{h.last_ambient_transcript && <div className="muted small">Ambient: {h.last_ambient_transcript}</div>}{h.last_command_redecode?.command_text && <div className="muted small">Command re-decode: {h.last_command_redecode.command_text}</div>}{h.last_rejection_reason && <div className="micWarn">Last rejection: {h.last_rejection_reason}</div>}</section>;
}
