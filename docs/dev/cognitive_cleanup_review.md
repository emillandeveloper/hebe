...
| `hebe_engine.py` | `_plan_and_execute_local_app_action` compatibility branch | Executes app actions for incomplete legacy/test engines | Compatibility only | Yes; requires `pc.open_application` | Low | Remove when all harnesses construct the real deliberation stack. |
